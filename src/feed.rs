...
pub fn fetch_feed(url: String) -> impl Future<Item = Feed, Error = ()> {
  fetch_feed_data(&url)
    ...
    .and_then(|data| consume_feed_data(&data, url))
...
// pub fn update_feed(channel_url: String) -> impl Future<Item = Vec<Item>, Error = ()> {
//   let local = channel_url.clone();
//   fetch_feed_data(channel_url)
//     .and_then(|data| identify_fetched_data(&data))
//     .and_then(move |data| consume_feed_data(data, &local))
      ...
      Ok(Event::Start(ref e)) => match e.name() {
        b"rss" => {
          ...
          match rss::Channel::read_from(BufReader::new(string)) {
            Ok(channel) => return futures::future::ok(FeedType::RSS(channel)),
        ...
        b"feed" => {
          ...
          match atom_syndication::Feed::read_from(BufReader::new(string)) {
            Ok(feed) => return futures::future::ok(FeedType::Atom(feed)),
            Err(_) => return futures::future::err(()),
          }
        }
        _ => (),
